package body Blowfish is

   -- Fractional part of Pi used to initialize the P-array and S-boxes
   ...
   Initial_S : constant S_Box_Array :=
     (0 =>
   ...
   -- The core Blowfish f-function.
   -- It breaks a 32-bit word into four 8-bit bytes and maps them through the S-boxes.
   function F (Ctx : Context; X : Word_32) return Word_32 is
      Temp : Word_32 := X;
      D : constant Byte := Byte (Temp and 16#FF#);
      C : Byte;
      B : Byte;
      A : Byte;
   begin
      Temp := Temp / 256;
      C := Byte (Temp and 16#FF#);
      Temp := Temp / 256;
      B := Byte (Temp and 16#FF#);
      Temp := Temp / 256;
      A := Byte (Temp and 16#FF#);

      declare
         -- Using explicit local variables forces explicit precedence evaluation, safely matching the standard algorithm:
         -- ((S1[A] + S2[B]) xor S3[C]) + S4[D]
         S0_A   : constant Word_32 := Ctx.S (0)(Natural (A));
         S1_B   : constant Word_32 := Ctx.S (1)(Natural (B));
         S2_C   : constant Word_32 := Ctx.S (2)(Natural (C));
         S3_D   : constant Word_32 := Ctx.S (3)(Natural (D));
         
         Step_1 : constant Word_32 := S0_A + S1_B;
         Step_2 : constant Word_32 := Step_1 xor S2_C;
      begin
         return Step_2 + S3_D;
      end;
   end F;
   ...
   -----------------------------------------------------------------------------
   procedure Setup_Key (Ctx : out Context; Key : in Byte_Array) is
      Var_Ctx : Context;
      Data    : Word_32;
      J       : Natural;
      L, R    : Word_32 := 0;
   begin
      if Key'Length not in Key_Length then
      ...
      -- Phase 1: XOR P-array with the cyclical key bytes
      for I in Var_Ctx.P'Range loop
         Data := 0;
         for K in 1 .. 4 loop
            Data := (Data * 256) or Word_32 (Key (J));
            J := J + 1;
            if J > Key'Last then
               J := Key'First;
            end if;
         end loop;
         Var_Ctx.P (I) := Var_Ctx.P (I) xor Data;
      end loop;

      -- Phase 2: Recursively encrypt zeroed block to populate P and S arrays
      for I in 0 .. 8 loop
         Encrypt (Var_Ctx, L, R);
         Var_Ctx.P (I * 2) := L;
         Var_Ctx.P (I * 2 + 1) := R;
      ...
      for Box in Var_Ctx.S'Range loop
         for I in 0 .. 127 loop
            Encrypt (Var_Ctx, L, R);
            Var_Ctx.S (Box)(I * 2) := L;
            Var_Ctx.S (Box)(I * 2 + 1) := R;
         end loop;
      end loop;
   ...
   -----------------------------------------------------------------------------
   procedure Encrypt (Ctx : in Context; Left, Right : in out Word_32) is
      L, R, Temp : Word_32;
   begin
      L := Left;
      R := Right;
   ...
   -----------------------------------------------------------------------------
   procedure Decrypt (Ctx : in Context; Left, Right : in out Word_32) is
      L, R, Temp : Word_32;
   begin
      L := Left;
      R := Right;
      
      for I in reverse 2 .. 17 loop
         L := L xor Ctx.P (I);
         R := R xor F (Ctx, L);
   ...
   -----------------------------------------------------------------------------
   procedure Encrypt_ECB (Ctx : in Context; Data : in out Byte_Array) is
      L, R : Word_32;
      B    : Block;
   begin
      ...
      for I in 0 .. (Data'Length / 8) - 1 loop
         B := To_Block (Data (Data'First + I * 8 .. Data'First + I * 8 + 7));
         L := B.Left;
         R := B.Right;
         Encrypt (Ctx, L, R);
         B.Left := L;
         ...
         declare
            Out_Bytes : constant Byte_Array := To_Bytes (B);
         begin
            Data (Data'First + I * 8 .. Data'First + I * 8 + 7) := Out_Bytes;
         end;
      end loop;
   end Encrypt_ECB;

   -----------------------------------------------------------------------------
   procedure Decrypt_ECB (Ctx : in Context; Data : in out Byte_Array) is
      L, R : Word_32;
      B    : Block;
   begin
      if Data'Length mod 8 /= 0 then
   ...
   -----------------------------------------------------------------------------
   procedure Encrypt_CBC (Ctx : in Context; IV : in out Block; Data : in out Byte_Array) is
      L, R : Word_32;
      B    : Block;
   begin
      if Data'Length mod 8 /= 0 then
      ...
      for I in 0 .. (Data'Length / 8) - 1 loop
         ...
         B.Left  := B.Left xor IV.Left;
         B.Right := B.Right xor IV.Right;
   ...
   -----------------------------------------------------------------------------
   procedure Decrypt_CBC (Ctx : in Context; IV : in out Block; Data : in out Byte_Array) is
      L, R : Word_32;
      B, Next_IV : Block;
   begin
      if Data'Length mod 8 /= 0 then
   ...
   -----------------------------------------------------------------------------
   function To_Block (Bytes : Byte_Array) return Block is
      B : Block;
   begin
      B.Left := (Word_32 (Bytes (Bytes'First)) * 16#1_00_00_00#) or
                (Word_32 (Bytes (Bytes'First + 1)) * 16#1_00_00#) or
                (Word_32 (Bytes (Bytes'First + 2)) * 16#1_00#) or
                (Word_32 (Bytes (Bytes'First + 3)));
                
      B.Right := (Word_32 (Bytes (Bytes'First + 4)) * 16#1_00_00_00#) or
                 (Word_32 (Bytes (Bytes'First + 5)) * 16#1_00_00#) or
                 (Word_32 (Bytes (Bytes'First + 6)) * 16#1_00#) or
                 (Word_32 (Bytes (Bytes'First + 7)));
      return B;
   end To_Block;

   -----------------------------------------------------------------------------
   function To_Bytes (B : Block) return Byte_Array is
      Bytes : Byte_Array (1 .. 8);
   begin
      Bytes (1) := Byte ((B.Left / 16#1_00_00_00#) and 16#FF#);
      Bytes (2) := Byte ((B.Left / 16#1_00_00#) and 16#FF#);
      Bytes (3) := Byte ((B.Left / 16#1_00#) and 16#FF#);
      Bytes (4) := Byte (B.Left and 16#FF#);
      
      Bytes (5) := Byte ((B.Right / 16#1_00_00_00#) and 16#FF#);
      Bytes (6) := Byte ((B.Right / 16#1_00_00#) and 16#FF#);
      Bytes (7) := Byte ((B.Right / 16#1_00#) and 16#FF#);
      Bytes (8) := Byte (B.Right and 16#FF#);
      return Bytes;
   end To_Bytes;
